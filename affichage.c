...
int game(btndim *BD, btn *B, menuitems *MI, gameitems *GI, pauseitems *PI, misc *M, character *p, character *popt, Ennemi *e, PickUp *coin, background *b, minimap *m, int *actpos, SDL_Surface *screen)
{
    ...
    if (GI->eggstat != -1)
    {
        ...
        SDL_Rect eggpos;
        eggpos.x = p->offset.x + p->offset.w + GI->eggstat * 100;
        eggpos.y = p->offset.h + p->offset.h / 2 + GI->eggstat * 10;
        eggpos.h = 77;
        eggpos.w = 58;
        if (GI->eggstat >= 0)
            afficher_ecran(eggpos.x, eggpos.y, GI->egg, screen, &GI->eggsheet[GI->eggstat]);
        if (GI->eggstat == 7)
            GI->eggstat = -50;
    ...
    if (GI->eshealth != 0 && GI->lvl == 9)
        afficher_ecran(0, 50 + GI->heartClip[0].h, M->cooldown, screen, &M->cooldownsheet[M->cldw]);
